...
import 'library.dart' show check;
...
class BufferWriter {
  ...
  /// advanced by [bytes.length].
  void set(final Iterable<int> bytes) {
    buffer.setAll(offset, bytes);
    offset += bytes.length;
  }
  
  /// Converts an integer [value] to a byte array of size [length] in [endian] format.
  List<int> convert(final int value, final int length, [final Endian endian = Endian.little]) {
    final Uint8List bytes =  buffer._toBytes(value, length);
    return endian == Endian.big ? bytes.reversed.toList(growable: false) : bytes;
  }
...
class Buffer extends Iterable<int> {
  ...
  /// Creates a [Buffer] of size [length] with random values in the range of `[0:maxUint8]`.
  /// 
  ...
  factory Buffer.random(
    final int length, [
    final Random? random,
  ]) => Buffer.generate(length, (_) => (random ?? Random.secure()).nextInt(maxUint8 + 1));
  ...
  int setUint(
    ...
    check(value >= 0, 'The [int] value $value must be a positive integer.');
  ...
  int setBigUint(
    ...
    check(value >= BigInt.zero, 'The [BigInt] value $value must be a positive integer.');
  ...
  Uint8List _toBytes(int value, final int length) {
    ...
    for (int i = 0; i < bytes.length && value != 0; ++i) {
      bytes[i] = value & maxUint8;
      value >>= _bits;
    }
    check(value <= 0, 'The [int] value $value overflows ${bytes.length} byte(s).');
    return bytes;
  }
  ...
  Uint8List _toBytesBigInt(BigInt value, [final int? length]) {
    final BigInt mask = BigInt.from(Buffer.maxUint8);
    ...
    check(value <= BigInt.zero, 'The [BigInt] value $value overflows ${bytes.length} byte(s).');
    return bytes;
  }
